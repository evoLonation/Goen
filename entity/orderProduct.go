...
var orderProductRepo entityRepo.RepoForEntity[OrderProduct]
var OrderProductRepo entityRepo.RepoForOther[OrderProduct]
...
type OrderProduct interface {
	...
	RemoveContainedEntries(orderEntry OrderEntry)
}
...
func (p *OrderProductEntity) GetSupplier() Supplier {
	if p.SupplierGoenId == nil {
		...
		ret, err := supplierRepo.Get(*p.SupplierGoenId)
		if err != nil {
			panic(err)
		}
		return ret
...
func (p *OrderProductEntity) GetContainedEntries() []OrderEntry {
	ret, _ := orderEntryRepo.FindFromMultiAssTable("order_product_contained_entries", p.GoenId)
	return ret
}
...
func (p *OrderProductEntity) SetSupplier(supplier Supplier) {
	id := supplierRepo.GetGoenId(supplier)
	p.SupplierGoenId = &id
	p.AddAssFieldChange("supplier_goen_id")
}
func (p *OrderProductEntity) AddContainedEntries(orderEntry OrderEntry) {
	p.AddMultiAssChange(entityRepo.Include, "order_product_contained_entries", orderEntryRepo.GetGoenId(orderEntry))
}
func (p *OrderProductEntity) RemoveContainedEntries(orderEntry OrderEntry) {
	p.AddMultiAssChange(entityRepo.Exclude, "order_product_contained_entries", orderEntryRepo.GetGoenId(orderEntry))
}
