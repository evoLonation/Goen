...
var cashDeskRepo entityRepo.RepoForEntity[CashDesk]
var CashDeskRepo entityRepo.RepoForOther[CashDesk]
...
type CashDesk interface {
	...
	RemoveContainedSales(sale Sale)
}
...
func (p *CashDeskEntity) GetContainedSales() []Sale {
	ret, _ := saleRepo.FindFromMultiAssTable("cash_desk_contained_sales", p.GoenId)
	return ret
}
...
func (p *CashDeskEntity) AddContainedSales(sale Sale) {
	p.AddMultiAssChange(entityRepo.Include, "cash_desk_contained_sales", saleRepo.GetGoenId(sale))
}
func (p *CashDeskEntity) RemoveContainedSales(sale Sale) {
	p.AddMultiAssChange(entityRepo.Exclude, "cash_desk_contained_sales", saleRepo.GetGoenId(sale))
}
